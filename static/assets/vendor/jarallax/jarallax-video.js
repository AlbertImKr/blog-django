...
(function (global, factory) {
    typeof exports === 'object' && typeof module !== 'undefined'
        ...
        typeof define === 'function' && define.amd ? define(factory) :
            (global = typeof globalThis !== 'undefined' ? globalThis : global
                || self, global.jarallaxVideo = factory());
})(this, (function () {
    'use strict';
    ...
    class VideoWorkerBase {
        ...
        fire(name, ...args) {
            if (this.userEventsList && typeof this.userEventsList[name]
                !== 'undefined') {
    ...
    function onAPIready$1(callback) {
        ...
        if ((typeof global$1$1.YT === 'undefined' || global$1$1.YT.loaded === 0)
        ...
        } else if (typeof global$1$1.YT === 'object' && global$1$1.YT.loaded
            === 1) {
            callback();
        } else {
    ...
    class VideoWorkerYoutube extends VideoWorkerBase {
        ...
        play(start) {
            ...
            if (global$1$1.YT.PlayerState.PLAYING
        ...
        getImageURL(callback) {
            ...
            const availableSizes = ['maxresdefault', 'sddefault', 'hqdefault',
                '0'];
            let step = 0;
            const tempImg = new Image();
            tempImg.onload = function () {
                // if no thumbnail, youtube add their own image with width = 120px
                if ((this.naturalWidth || this.width) !== 120 || step
        ...
        getVideo(callback) {
            ...
            onAPIready$1(() => {
                ...
                self.playerOptions.events = {
                    onReady(e) {
                        ...
                        if (self.options.loop && !self.options.endTime) {
                            ...
                            self.options.endTime = self.player.getDuration()
                                - secondsOffset;
                        }
                    ...
                    onStateChange(e) {
                        // loop
                        if (self.options.loop && e.data
                            === global$1$1.YT.PlayerState.ENDED) {
                            self.play(self.options.startTime);
                        }
                        if (!ytStarted && e.data
                            === global$1$1.YT.PlayerState.PLAYING) {
                            ytStarted = 1;
                            self.fire('started', e);
                        }
                ...
                self.player = self.player || new global$1$1.YT.Player(
                    self.playerID, self.playerOptions);
                if (firstInit) {
                    ...
                    self.videoWidth = parseInt(
                        self.$video.getAttribute('width'), 10) || 1280;
                    self.videoHeight = parseInt(
                        self.$video.getAttribute('height'), 10) || 720;
                }
                callback(self.$video);
            });
        }
    ...
    class VideoWorkerVimeo extends VideoWorkerBase {
        ...
        getImageURL(callback) {
            ...
            request.open('GET',
                `https://vimeo.com/api/oembed.json?url=${self.url}&width=${width}`,
                true);
        ...
        getVideo(callback) {
            ...
            onAPIready(() => {
                ...
                self.playerOptions = {
                    ...
                    muted: self.options.mute || self.options.volume === 0 ? 1
                        : 0
                };
                ...
                if (!self.options.showControls && self.options.loop
                ...
                if (!self.$video) {
                    ...
                    Object.keys(self.playerOptions).forEach(key => {
                        ...
                        playerOptionsString += `${key}=${encodeURIComponent(
                            self.playerOptions[key])}`;
                    });
                ...
                self.player = self.player || new global$1$1.Vimeo.Player(
                    self.$video, self.playerOptions);

                // Since Vimeo removed the `volume` parameter, we have to set it manually.
                if (!self.options.mute && typeof self.options.volume
                    === 'number') {
                ...
                self.player.on('timeupdate', e => {
                    ...
                    // check for end of video and play again or stop
                    if (self.options.endTime && e.seconds
                        >= self.options.endTime) {
                        if (self.options.loop) {
                            self.play(self.options.startTime);
                        } else {
        ...
        getVideo(callback) {
            ...
            if (!self.$video) {
                ...
                Object.keys(self.videoID).forEach(key => {
                    addSourceElement(self.$video, self.videoID[key],
                        `video/${key}`);
                });
            }
            let locStarted;
            ...
            self.player.addEventListener('timeupdate', function (e) {
                ...
                // check for end of video and play again or stop
                if (self.options.endTime && this.currentTime
                    >= self.options.endTime) {
                    if (self.options.loop) {
                        self.play(self.options.startTime);
                    } else {
    ...
    function jarallaxVideo(jarallax = global$1.jarallax) {
        ...
        Jarallax.prototype.onScroll = function () {
            ...
            const isReady = !self.isVideoInserted && self.video
            ...
            if (isReady) {
                ...
                self.video.getVideo(video => {
                    ...
                    if (self.video.type === 'local') {
                        ...
                        } else if (self.image.$item && self.image.$item.tagName
                            === 'IMG' && self.image.$item.src) {
                            self.$video.setAttribute('poster',
                                self.image.$item.src);
                        }
                    }
        ...
        Jarallax.prototype.coverImage = function () {
            ...
            if (imageData && self.video && node && (node === 'IFRAME' || node
                === 'VIDEO')) {
        ...
        Jarallax.prototype.initImg = function () {
            ...
            if (!self.options.videoSrc) {
                self.options.videoSrc = self.$item.getAttribute(
                    'data-jarallax-video') || null;
            }
            if (self.options.videoSrc) {
                        ...
                        video.getImageURL(url => {
                            ...
                            if (curStyle) {
                                self.$item.setAttribute(
                                    'data-jarallax-original-styles', curStyle);
                            }
                    ...
                    video.on('ready', () => {
                        if (self.options.videoPlayOnlyVisible) {
                            ...
                            self.onScroll = function () {
                                ...
                                if (!self.videoError && (self.options.videoLoop
